...
void VulkanLaunchFuncToVulkanCallsPass::declareVulkanFunctions(Location loc) {
  ...
  if (!module.lookupSymbol(kBindMemRef1DInt)) {
    builder.create<LLVM::LLVMFuncOp>(
        loc, kBindMemRef1DInt,
        LLVM::LLVMType::getFunctionTy(getVoidType(),
                                      {getPointerType(), getInt32Type(),
                                       getInt32Type(),
                                       getMemRef1DInt().getPointerTo()},
                                      /*isVarArg=*/false));
  }

  if (!module.lookupSymbol(kBindMemRef2DInt)) {
    builder.create<LLVM::LLVMFuncOp>(
        loc, kBindMemRef2DInt,
        LLVM::LLVMType::getFunctionTy(getVoidType(),
                                      {getPointerType(), getInt32Type(),
                                       getInt32Type(),
                                       getMemRef2DInt().getPointerTo()},
                                      /*isVarArg=*/false));
  }

  if (!module.lookupSymbol(kBindMemRef3DInt)) {
    builder.create<LLVM::LLVMFuncOp>(
        loc, kBindMemRef3DInt,
        LLVM::LLVMType::getFunctionTy(getVoidType(),
                                      {getPointerType(), getInt32Type(),
                                       getInt32Type(),
                                       getMemRef3DInt().getPointerTo()},
                                      /*isVarArg=*/false));
  }
